window.onload = function () {
    ...
    const getRandomLyrics = (arr) => {
        ...
        for (let i = 0; i < arr.length; i++) {
            ...
            document.getElementById("artwork").src = arr[random].artwork;
            // random viene usato come l'indice da cui deve'essere recuperato l'elemento da mostrare
            document.getElementById("lyrics").innerHTML = arr[random].lyric;
            document.getElementById("artist").innerHTML = `- ${arr[random].author}`;
            document.body.style.background = arr[random].background;
            document.getElementById("artwork").alt = arr[random].alt;
        };*/
